...
func ReadFile(fileName string) Blockchain {
	...
	buffer, _ := ioutil.ReadFile("blockchain.json") //Read file and fill the buffer
	json.Unmarshal(buffer, &blockchain)             //transform bytes into our structure
	return blockchain
}
...
func WriteFile(filename string, data string) {
	...
	file, err := os.OpenFile(filename, os.O_RDWR|os.O_CREATE, 0755) //os.Open opens file in ReadOnly mode
	Must(err)                                                       // os.O_RDWR set up the chmod for file
	writer := io.Writer(file)                                       //We need it to json.NewEncoder
	encoder := json.NewEncoder(writer)                              //NewEncoder returns encoder for json files
	blockchain.AddBlock(data)
	err = encoder.Encode(&blockchain) // write it to file
	Must(err)
}
